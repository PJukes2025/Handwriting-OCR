...
def image_content_hash(pil_image: Image.Image) -> str:
    """
    Compute a stable sha1 hash of image bytes (PNG-encoded) to disambiguate
    same-named files or files with no name.
    """
    with io.BytesIO() as buf:
        pil_image.save(buf, format="PNG")
        data = buf.getvalue()
    return hashlib.sha1(data).hexdigest()[:10]  # short hash for readability

def make_storage_key(filename: str, pil_image: Image.Image) -> str:
    """
    Build a stable storage key: <filename_or_UNNAMED>__<hash>
    This prevents collisions when two images share a name and
    ensures we can still store corrections when no filename exists.
    """
...
def learn_from_corrections(original_text, corrected_text, image_key):
    ...
    - Create/update an override if corrected_text is non-empty.
    """
    if 'user_corrections' not in st.session_state:
        st.session_state.user_corrections = {}
    ...
    entry = {
        ...
        'image': image_key,
        'original': (original_text or ""),     # allow empty
        'corrected': (corrected_text or ""),   # allow empty
...
def apply_learned_corrections(text, image_key):
    """Apply per-image override first; then generic fixes and any saved global replacements."""
    ...
    # 3) Any saved global replacements (use all overrides as lightweight replacements)
...
# ===================== Corrections Manager (Sidebar quick view) =====================
    ...
    for uploaded_file in files:
        try:
            ...
            result = {
                ...
                'storage_key': storage_key,        # stable key for corrections (filename__hash)
                'text': corrected_text,
                'original_text': best_text,
                'engine': f'Tesseract ({best_config})',
                'all_configs': all_configs,
            ...
            with st.expander("✏️ Edit & Save Correction"):
                ...
                with save_cols[0]:
                    if st.button("Save Correction", key=f"save_corr_{idx}"):
                        # Persist using stable storage_key
                        learn_from_corrections(res['original_text'], edited_text, res['storage_key'])
                        # Update in-memory result + UI immediately
    ...
    if selected_key:
        ...
        with mcols[0]:
            if st.button("Update Override", key="mgr_update"):
                ...
                # Use same storage key to append history and update override
                learn_from_corrections(prev_original, new_corrected, selected_key)
                # if image is visible in results, refresh it
