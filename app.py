...
# ===================== HARD INIT (must be first, before any early stops) =====================
...
ocr_lang = st.sidebar.selectbox("OCR Language", ["eng", "deu", "fra", "spa", "ita", "por", "nld"], index=0,
                                help="Language must be installed in Tesseract.")
...
# ===================== Corrections: store & apply =====================
def learn_from_corrections(original_text, corrected_text, image_key):
    """Always append to history; update override if corrected_text is non-empty."""
    if 'user_corrections' not in st.session_state:
        st.session_state['user_corrections'] = {}
...
def apply_learned_corrections(text, image_key):
    ...
    for wrong, right in journalism_fixes.items():
        corrected_text = corrected_text.replace(wrong, right)
    # Lightweight global replacements from overrides
    ...
    for idx, res in enumerate(results):
        ...
        with col_txt:
            ...
            with st.expander("✏️ Edit & Save Correction"):
                edited_text = st.text_area("Correct the text below and click Save.", value=res['text'], height=180, key=f"edit_text_{idx}")
                cols = st.columns(3)
                with cols[0]:
                ...
                with cols[1]:
                    if st.button("Re-OCR This Image", key=f"reocr_{idx}"):
                        ...
                        st.rerun()
                with cols[2]:
                    if st.button("Save DEMO correction (test)", key=f"save_demo_{idx}"):
                        demo_text = f"[DEMO OVERRIDE for {res['storage_key']}]"
                        learn_from_corrections(res.get('original_text',''), demo_text, res['storage_key'])
                        res['text'] = demo_text
                        st.session_state.ocr_results[idx] = res
                        st.session_state[f"view_text_{idx}"] = demo_text
                        st.session_state[f"edit_text_{idx}"] = demo_text
                        st.success("Demo correction saved.")
                        st.rerun()
...
st.info("🚀 For notebook photos, try **Enhancement = aggressive**. Use rotate buttons if orientation looks wrong.")

# ===================== File upload =====================
st.header("📤 Upload Images")
if batch_mode:
    uploaded_files = st.file_uploader("Choose image files (JPG, PNG, HEIC)", type=['jpg','jpeg','png','heic'],
...
if not ov_keys:
    ...
    if selected_key:
        ...
        with c2:
            ...
            new_corr = st.text_area("Edit corrected text & click Update", value=override.get('corrected', ''), height=140, key="mgr_edit_corr")

        mcols = st.columns(3)
        with mcols[0]:
            if st.button("Update Override", key="mgr_update"):
                prev_orig = override.get('original', '')
                learn_from_corrections(prev_orig, new_corr, selected_key)
                ...
                for i, r in enumerate(st.session_state.ocr_results):
                    if r.get('storage_key') == selected_key:
                        ...
                        st.session_state[f"view_text_{i}"] = new_corr
                        st.session_state[f"edit_text_{i}"] = new_corr
