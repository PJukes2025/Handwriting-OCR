...
# ===================== Cookie-less session fallback (global store via URL SID) =====================
@st.cache_resource
def _global_store():
    # server-side persistent dict across reruns (per process)
    return {}  # { sid: {"user_corrections": {}, "created_at": ...} }

def get_sid():
    # Use ?sid=... in URL if present; otherwise create one and set it
    qp = st.query_params
...
SID = get_sid()
if SID not in STORE:
    STORE[SID] = {"user_corrections": {}, "created_at": datetime.utcnow().isoformat()}

def sync_from_store():
    """Load corrections from global store -> session_state (first run or if empty)."""
    if 'user_corrections' not in st.session_state or not isinstance(st.session_state['user_corrections'], dict):
        st.session_state['user_corrections'] = {}
    if not st.session_state['user_corrections'] and STORE[SID].get('user_corrections'):
        st.session_state['user_corrections'] = dict(STORE[SID]['user_corrections'])

def sync_to_store():
    """Mirror session_state corrections -> global store, then rerun to reflect in UI."""
    STORE[SID]['user_corrections'] = dict(st.session_state.get('user_corrections', {}))

# ===================== HARD INIT for session_state =====================
...
# Load any existing corrections from cookie-less store
sync_from_store()
...
st.sidebar.caption(f"Session UID: `{st.session_state['session_uid']}`")
...
show_raw = st.sidebar.checkbox("Show raw corrections (debug)", value=False, key="show_raw_corrs")
if show_raw:
...
def learn_from_corrections(original_text, corrected_text, image_key):
    ...
    # mirror to cookie-less store and force rerun
    sync_to_store()
    st.rerun()
        ...
        with col_txt:
            ...
            with st.form(key=f"corr_form_{res['storage_key']}"):
                ...
                if submitted:
                    ...
                    # learn_from_corrections will sync_to_store() and st.rerun()
    ...
    if selected_key:
        ...
        with m1:
            if st.button("Update Override", key="mgr_update"):
                # append to history + update override
                entry_orig = override.get('original','')
                if not isinstance(entry_orig, str): entry_orig = ""
                # we reuse learn_from_corrections for consistency
                learn_from_corrections(entry_orig, new_corr, selected_key)
                # learn_from_corrections syncs + reruns
        with m2:
...
with st.expander("🧹 Danger zone: Clear ALL corrections"):
    if st.button("Clear ALL corrections now"):
        st.session_state['user_corrections'] = {}
        sync_to_store()
        st.success("All corrections cleared for this SID.")
        st.rerun()
