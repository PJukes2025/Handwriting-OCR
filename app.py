...
# ---------------- UI: per-image display (form submit saves to STORE only) ----------------
    ...
    for idx, res in enumerate(results):
        ...
        with col_txt:
            ...
            with st.form(key=f"corr_form_{res['storage_key']}"):
                ...
                if submitted:
                    corr_save(res['original_text'], edited_text, res['storage_key'])  # WRONG ORDER? -> Fix below
                    st.stop()  # will be replaced after fix
...
if not keys:
    ...
    if selected_key:
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("**Original (last saved)**"); st.code(ov.get('original','') or "(unknown)")
        with c2:
            st.markdown("**Corrected (applied)**")
            new_corr = st.text_area("Edit corrected text & Update", value=ov.get('corrected',''), height=140, key="mgr_edit_corr")
        m1, m2, m3 = st.columns(3)
        with m1:
            if st.button("Update Override", key="mgr_update"):
                corr_save(selected_key, ov.get('original',''), new_corr)  # append history + update override
                st.success("Override updated."); st.rerun()
        with m2:
            if st.button("Delete Override", key="mgr_delete"):
                corr_delete(selected_key, delete_history=False); st.success("Override deleted."); st.rerun()
        with m3:
            if st.button("Delete Override + History", key="mgr_delete_all"):
                corr_delete(selected_key, delete_history=True); st.success("Override and history deleted."); st.rerun()
