...
# Language selector (installed langs vary; 'eng' always available with tesseract-ocr)
ocr_lang = st.sidebar.selectbox(
    "OCR Language",
    ["eng", "deu", "fra", "spa", "ita", "por", "nld"],
    index=0,
    help="Requires language data installed on the server"
)
...
def preprocess_image(image, enhancement_level="medium"):
    ...
    # Noise removal
    denoosed = cv2.fastNlMeansDenoising(gray)

    # Enhance based on level
    if enhancement_level == "light":
        processed = cv2.threshold(denoosed, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
    elif enhancement_level == "medium":
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
        enhanced = clahe.apply(denoosed)
...
def display_results(results, enhancement_level, lang, show_processed_images=False, show_alternative_configs=False, debug_mode=False):
    ...
    for idx, res in enumerate(results):
        ...
        with col_txt:
            ...
            # --- Alternative configs output (when enabled or in debug) ---
...
if st.session_state.ocr_results:
    ...
    if st.button("Re-run Now"):
        ...
        rerun_results = process_images(images_for_rerun, enhancement_level, ocr_lang)
